...
public:
    ...
    CRGB multiply(CRGB C)
    {
		unsigned char R = clamp(m_Red   * C.m_Red, 0, 255); 
		unsigned char G = clamp(m_Green * C.m_Green, 0, 255);
		unsigned char B = clamp(m_Blue  * C.m_Blue, 0, 255); 
		
		return CRGB( R, G, B );
    }

    CRGB average(CRGB &C1, CRGB &C2)
    {
		unsigned char R = clamp((C1.m_Red   + C2.m_Red)/2, 0, 255); 
		unsigned char G = clamp((C1.m_Green + C2.m_Green)/2, 0, 255);
		unsigned char B = clamp((C1.m_Blue  + C2.m_Blue)/2, 0, 255); 
